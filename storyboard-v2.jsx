...
const STYLES = ["광고 / Commercial","브이로그 / Vlog","영화 / Cinematic","뮤직비디오 / MV","다큐 / Documentary"];
const MOODS  = ["럭셔리 / Luxury","감성 / Emotional","레트로 / Retro","미니멀 / Minimal","다이나믹 / Dynamic","드라마틱 / Dramatic"];
...
const DURATION_PRESETS = [
  {label:"15초",val:15},{label:"30초",val:30},{label:"1분",val:60},
  {label:"2분",val:120},{label:"3분",val:180},{label:"직접입력",val:0},
];
...
const fmtDur = s => `${Math.round(s * 10) / 10}s`;

function calcCuts(total, maxCut) {
...
// ── SMALL COMPONENTS ───────────────────────────────────────────────────────
function Spin({ size = 16, color = "rgba(255,255,255,0.7)" }) {
  return (
    <div style={{
...
function SelectInput({ label, value, setter, options, accent }) {
  return (
    <div>
      <label style={{ display:"block", fontSize:10, color: accent || "rgba(251,191,36,0.65)", letterSpacing:1.5, marginBottom:7 }}>{label}</label>
...
function BeatCard({ beat, cutNumber, cutTimeStart, topic, solution }) {
  ...
  const [vidPrompt, setVidPrompt] = useState(null);
  ...
  const imgGen = () =>
    sendPrompt(`스토리보드 이미지 생성:\n\n[CUT ${String(cutNumber).padStart(2,"0")} · ${beat.beatId}]\n절대시간: ${fmtSec(absStart)}–${fmtSec(absEnd)}\n\n${beat.firstFrame}\n\nHiggsfield로 이미지 만들어줘.`);
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ fontFamily:"'Space Mono',monospace", fontSize:13, fontWeight:700, color:"#A78BFA" }}>{beat.beatId}</span>
      ...
      <div style={{ padding:"10px 13px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
          {[["🎥",beat.cameraAction,"rgba(255,255,255,0.55)"],
            ...
            ["🎵",beat.audioNote,"rgba(255,255,255,0.45)"]
      ...
      <div style={{ padding:"0 13px 13px", display:"flex", flexWrap:"wrap", gap:5 }}>
        <button onClick={imgGen} style={{
          ...
          padding:"6px 8px", fontSize:10, fontWeight:700, color:"#DDD6FE", cursor:"pointer",
        }}>🎨 이미지 생성</button>
...
function CutCard({ cut, meta, beatBoard, isGenBeats, onGenBeat, idx }) {
  ...
  const em = EMOTION_MAP[cut.emotion] || EMOTION_MAP.Establishing;

  const imgGen = (prompt, label) =>
    sendPrompt(`스토리보드 이미지 생성:\n\n[${label}]\n${fmtSec(cut.timeStart)}–${fmtSec(cut.timeEnd)}\n\n${prompt}\n\nHiggsfield로 퍼스트프레임 이미지 만들어줘.`);
    ...
    <div style={{
      ...
      <div style={{ display:"grid", gridTemplateColumns:"minmax(260px,1fr) 1fr 1fr", gap:0 }}>
        ...
        <div style={{ padding:18, borderRight:"1px solid rgba(255,255,255,0.045)" }}>
          ...
          <div onClick={() => setShowPrompt(!showPrompt)} style={{
            aspectRatio:"16/9", background:"rgba(0,0,0,0.35)",
            borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center",
            marginBottom:10, cursor:"pointer", border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden",
          }}>
            {showPrompt ? (
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
            <button onClick={() => imgGen(cut.firstFrame, `CUT ${String(cut.cutNumber).padStart(2,"0")}`)} style={{
              flex:"1 1 100%",
              background:"linear-gradient(135deg,#FBBf24,#F59E0B)",
              border:"none", borderRadius:8, padding:"9px 12px",
              fontSize:11, fontWeight:800, color:"#000", cursor:"pointer", letterSpacing:0.5,
            }}>🎨 이미지 생성</button>
...
export default function App() {
  ...
  const [beatBoards,     setBeatBoards]     = useState({});
  const [loading,        setLoading]        = useState(false);
  const [genBeats,       setGenBeats]       = useState({});
...
[{
  ...
  "audioNote":"Korean: BGM / SFX suggestion for this beat"
    ...
    const user = `Break CUT ${String(cut.cutNumber).padStart(2,"0")} (${fmtDur(cut.duration)}) into ${beatCount} micro beats.
Scene: ${cut.content}
...
- Show natural micro-progression of camera and emotion through the ${cut.duration}s`;
    ...
    } catch { showToast("비트 생성 실패. 다시 시도해주세요."); }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#090912", fontFamily:"'Noto Sans KR',sans-serif", color:"#E8E8F2" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700&family=Bebas+Neue&family=Space+Mono:wght@400;700&display=swap');
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1440, margin:"0 auto", padding:"0 28px 100px" }}>
        ...
        <div style={{ padding:"34px 0 24px", display:"flex", alignItems:"center", gap:18, borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
            ...
            ].map(([lvl,name,bg,color]) => (
              <div key={lvl} style={{
                padding:"6px 12px", background:bg, borderRadius:8,
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <input value={topic} onChange={e=>setTopic(e.target.value)}
              ...
              style={{
                ...
                padding:"13px 16px", fontSize:14, color:"#E8E8F2",
                transition:"border-color 0.2s",
              }}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1.2fr 1fr 1.4fr 1fr", gap:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:8 }}>
                {DURATION_PRESETS.map(p => (
                  <button key={p.label} onClick={()=>{
                    ...
                    background:(!useCustom&&totalDuration===p.val)||(useCustom&&p.val===0)
                      ?"rgba(251,191,36,0.18)":"rgba(255,255,255,0.05)",
                    border:`1px solid ${(!useCustom&&totalDuration===p.val)||(useCustom&&p.val===0)
                      ?"rgba(251,191,36,0.45)":"rgba(255,255,255,0.07)"}`,
                    color:(!useCustom&&totalDuration===p.val)||(useCustom&&p.val===0)
                      ?"#FBBf24":"rgba(255,255,255,0.45)",
                  }}>{p.label}</button>
                ))}
              </div>
              {useCustom && (
                <input type="number" placeholder="초 단위" value={customDur}
            ...
            <div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                <input type="range" min={solInfo.min} max={solInfo.max} step={1}
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <textarea value={extraNote} onChange={e=>setExtraNote(e.target.value)} rows={2}
              ...
              style={{
                ...
                padding:"11px 15px", fontSize:13, color:"#E8E8F2",
          ...
          <div style={{ marginTop:36, animation:"fadeInUp 0.5s ease" }}>
                  ...
                  <div style={{ flex:"1 1 300px" }}>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                      {storyboard.copyLines.map((l,i) => (
                        <span key={i} onClick={()=>{navigator.clipboard.writeText(l);showToast("복사됐어요!");}}
                          style={{
            ...
            <div style={{ marginBottom:24 }}>
              ...
              <div style={{
                ...
                {storyboard.cuts.map(cut => {
                  ...
                  return (
                    <div key={cut.cutNumber} title={`CUT ${String(cut.cutNumber).padStart(2,"0")} · ${fmtSec(cut.timeStart)}–${fmtSec(cut.timeEnd)}`}
                      style={{
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                {[
                  Object.keys(beatBoards).length === 0 && { label:"모든 컷 비트보드 생성", color:"#A78BFA", bg:"rgba(167,139,250,0.12)", border:"rgba(167,139,250,0.25)",
                    action: () => storyboard.cuts.forEach((c, i) => setTimeout(()=>generateBeat(c), i*1200)) },
                ].filter(Boolean).map((btn,i) => (
                  <button key={i} onClick={btn.action} style={{
                    padding:"8px 16px", background:btn.bg, border:`1px solid ${btn.border}`,
                    borderRadius:9, fontSize:11, fontWeight:700, color:btn.color, cursor:"pointer",
                  }}>{btn.label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {storyboard.cuts.map((cut, idx) => (
                <CutCard
                  ...
                  beatBoard={beatBoards[cut.cutNumber]}
